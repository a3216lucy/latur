...
var goBtn = document.getElementById('go'); //取得去程
var backBtn = document.getElementById('back');//取得回程
var busList = document.getElementById('bus-way-list'); //公車指定班次的路線列表

goBtn.addEventListener('click', getGoJson, false); //點擊去程按鈕,會啟動監聽動作=>getGoJson
backBtn.addEventListener('click', getBackJson, false);//點擊回程按鈕,會啟動監聽動作=>getGoJson

var helper = {
    getParameterByName: function (name, url) {
        var regex, results;
        if (!url) {
            url = window.location.href;  //取得位置定位
        }
        name = name.replace(/[\[\]]/g, '\\$&');
...
/* 去程資料 */
function getGoJson(){
    clearInterval(getBackJson); // 先顯示去程資料，而讓畫面不會渲染出 回程路線
    check = "go";

    var xhr = new XMLHttpRequest();
    xhr.open('get', GoUrl); //取得PTX資料
    xhr.send(null);
    xhr.onload = function () {
